...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadInitialRecipes() async {
    ...
    try {
      ...
      if (categoryService.categories.isEmpty) {
        await categoryService.fetchCategories(forceRefresh: false);
      }
  ...
  Future<void> _loadRecipesWithFilters() async {
    ...
    try {
      ...
      await Future.wait([
        categoryService.fetchCategories(forceRefresh: false),
  ...
  AppBar _buildAppBar(AuthService authService) {
    return AppBar(
      title:
          const Text('OrsoCook', style: TextStyle(fontWeight: FontWeight.bold)),
      actions: [
        IconButton(
  ...
  Widget build(BuildContext context) {
    return Consumer2<RecipeService, AuthService>(
      builder: (context, recipeService, authService, child) {
        ...
        if (_isFirstLoad || (isLoading && recipes.isEmpty)) {
          return Scaffold(
